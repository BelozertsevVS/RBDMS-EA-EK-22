...
BEGIN
    ...
    DECLARE @CurrentSales DECIMAL(15, 2)
    -- Отримання ціни товару:
    SELECT @Price = PRICE
    FROM dbo.PRODUCTS
    WHERE PRODUCT_ID = @ProductID
--Початковий етап завершено.
--І. Перевірка наявності кількості товару на складі:
    IF (SELECT QTY_ON_HAND FROM dbo.PRODUCTS WHERE PRODUCT_ID = @ProductID) >= @Qnty
    BEGIN
--I етап завершено.
--ІІ. Розрахунок суми замовлення:
        SET @TotalAmount = @Price * @Qnty
-- Отримання кредитного ліміту клієнта:
        SELECT @CreditLimit = CREDIT_LIMIT
        FROM dbo.CUSTOMERS
        WHERE CUST_NUM = @CustomerID
-- Отримання поточних продажів робітника:
        SELECT @CurrentSales = SALES
        FROM dbo.SALESREPS
        WHERE EMPL_NUM = @EmployeeID
--II етап завершено.
--III. Перевірка кредитного ліміту:
...
-- Зменшення кількості товару на складі:
            UPDATE dbo.PRODUCTS
            SET QTY_ON_HAND = QTY_ON_HAND - @Qnty
            WHERE PRODUCT_ID = @ProductID
-- Вирахування з кредитного ліміту:
...
-- Внесення замовлення до таблиці замовлень:
            ...
            VALUES ( @CustomerID, @EmployeeID, @ManufacturerID, @ProductID, @Qnty, @TotalAmount)
-- Оновлення поточних продажів робітника та офісу:
