...
SELECT DATEDIFF(year, [HIRE_DATE], GETDATE()) AS [WORK_YEAR], COUNT(*) AS [NUM_EMPL]
FROM [dbo].[SALESREPS]
...
SELECT TOP 1 
WITH TIES YEAR([HIRE_DATE]) AS [HIRE_YEAR], MONTH([HIRE_DATE]) AS [HIRE_MONTH], COUNT(*) AS [NUM_EMPL]
FROM [dbo].[SALESREPS]
...
SELECT DATENAME(WEEKDAY,[ORDER_DATE]) AS [DAY_OF_WEEK] ,DATEPART(WEEKDAY,[ORDER_DATE]) AS [DATE_PART], COUNT(DISTINCT [PRODUCT]) AS [UN_PR], SUM([AMOUNT]) AS [GEN_SUM_AMOUNT], SUM([QTY]) AS [GEN_QTY_SUM]
FROM [dbo].[ORDERS]
