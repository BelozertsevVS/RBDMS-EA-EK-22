...
SELECT*FROM SALESREPS
SELECT YEAR([HIRE_DATE]) AS Рік_найму,
...
SELECT DATENAME(WEEKDAY, [ORDER_DATE]) AS Номер_тижня,
DATEPART(WEEKDAY, [ORDER_DATE]) AS Назва_дня_тижня,
COUNT(DISTINCT [PRODUCT]) AS Проведені_замовлення,
SUM([AMOUNT]) AS Сума_продаж,
SUM([QTY]) AS Кількість_проданих_тов
FROM [dbo].[ORDERS]
