...
-- Створюємо таблицю SALES для зберігання даних замовлення, з відповідними типами даних
    CREATE TABLE SALES (
        SALES_ORDER_ID  BIGINT PRIMARY KEY, -- Тип даних bigint призначений для використання, коли цілі значення можуть перевищувати діапазон, який підтримується типом даних int .
        REVISION_NUMBER  TINYINT,-- TINYINT для інкременту (до 255)
        ORDER_DATE  DATETIME,
        DUE_DATE  DATE,
		SHIP_DATE DATE,
		ONLINE_ORDER_FLAG BIT, -- Цілочисленний тип даних, який може набувати значення 1, 0 або NULL.
		CUSTOMER_ID INT,
		SALES_PERSON_ID INT,
		TERRITORY_ID INT,
		SUB_TOTAL MONEY,
...
-- Вносимо дані у створену таблицю
...
-- Відбір всіх даних з таблиці
SELECT * FROM SALES;

-- Видалення таблиці SALES
DROP TABLE SALES;

-- Видалення бази даних SHOP1
USE master; -- Переключення на майстер-базу перед видаленням
