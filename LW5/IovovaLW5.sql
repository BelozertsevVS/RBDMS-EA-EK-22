...
--Створимо таблицю в БД SHOP для зберігання даних замовлень. Визначимо найбільш оптимальні типи даних:
--Свторимо таблицю Продажі:
CREATE TABLE SALES(
SALES_ORDER_ID BIGINT,  --велике число
REVISION_NUMBER TINYINT,  --мале число(до 255)
ORDER_DATE DATETIME,  --дата і час
DUE_DATE DATE,  --дата
SHIP_DATE DATE,  --дата
ONLINE_ORDER_FLAG TINYINT CHECK (ONLINE_ORDER_FLAG IN (0, 1)),  --статус
CUSTOMER_ID INT ,  --число
SALES_PERSON_ID INT,  --число
TERRITORY_ID INT,  --число
SUB_TOTAL MONEY, --грошова величина
TAX_AMT SMALLMONEY,  --менша грошова величина
FREIGHT SMALLMONEY,  --менша грошова величина
MODIFIED_DATE DATETIME DEFAULT GETDATE()  --Дата і час останньої зміни рядка
);

--Виконаймо запит для внесення даних до створеної таблиці:
...
VALUES (43659, 11, '1753-01-01 00:00:00.000', '2011-06-12', '2011-06-07', 0, 29825,	279, 5,	184590.1346, 1971.5149	,616.0984	        ,'2011-06-07 00:00:00.000'),
	   ...
	   --Відіберемо з таблиці всі дані:
	   SELECT * FROM SALES;
	   --Дані відібрано коректно, усе працює


	   --Видалимо таблицю SALES:
	   DROP TABLE SALES;
	   --Таблицю SALES видалено успішно


	   --Видалимо БД SHOP1:
	   DROP DATABASE SHOP1;
	   --БД SHOP1 видалено успішно
	   --Робота виконана