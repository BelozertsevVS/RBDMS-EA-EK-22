...
SELECT RTRIM(CONCAT_WS(' ', [MFR_ID], [PRODUCT_ID], UPPER([DESCRIPTION]))) AS [CONCAT_LINE],
       LEN(RTRIM(CONCAT_WS(' ', [MFR_ID], [PRODUCT_ID], UPPER([DESCRIPTION])))) AS [LINE_LEN]
FROM [dbo].[PRODUCTS]
...
SELECT TOP 1
WITH TIES [CUST_NUM]
         ,SUBSTRING([COMPANY], 1, 2) + REPLICATE('*', LEN(COMPANY) - 4) + SUBSTRING([COMPANY], LEN([COMPANY]) - 1, 2) AS [COMPANY*]
		 ,[CREDIT_LIMIT]
FROM [dbo].[CUSTOMERS]
