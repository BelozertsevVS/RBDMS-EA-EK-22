...
SELECT [MFR] AS [MFR_ID], SUM([AMOUNT]) AS [TOTAL_SALES]
FROM [dbo].[ORDERS]
...
SELECT DATEDIFF(YEAR,[HIRE_DATE], GETDATE()) AS [YEARS_OF_SERVICE], COUNT([EMPL_NUM] ) AS [COUNT_OF_EMPL]
FROM [dbo].[SALESREPS]
...
SELECT YEAR([HIRE_DATE]) AS [HIRE_YEAR], MONTH([HIRE_DATE]) AS [HIRE_MONTH], COUNT([HIRE_DATE]) AS [HIRED_COUNT]
FROM [dbo].[SALESREPS]
