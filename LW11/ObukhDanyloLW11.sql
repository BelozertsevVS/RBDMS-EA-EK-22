...
SELECT [MFR_ID], ROUND(SUM([PRICE][QTY_ON_HAND]),2) AS [GEN_COST], ROUND(AVG([PRICE][QTY_ON_HAND]),2) AS [AVG_COST]
FROM [dbo].[PRODUCTS]
GROUP BY [MFR_ID]

/* Задача 3
Напишіть запит, який поверне працівника з найбільшою мат. різницею між ціллю за продажами і сумою поточних продаж, у абсолютному вимірі, тобто необхідно взяти число за модулем. 
Округлити результати обчислення до двох знаків після коми. Розрахувати кількість років працівника на момент найму.
...
SELECT TOP 1
WITH TIES [NAME] ,ABS(ROUND(([QUOTA]-[SALES]),2)) AS [DIV_SALES_GOAL], [AGE] - DATEDIFF(year, [HIRE_DATE], GETDATE()) AS [HIRE_AGE]
FROM [dbo].[SALESREPS]
