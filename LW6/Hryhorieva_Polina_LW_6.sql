...
--Введення даних до `Departments`. Якщо `DepartmentName` не вказано, використовується значення за замовчуванням
...
--Спроба вставлення запису до `Professors` з існуючим `Email`, щоб перевірити UNIQUE обмеження.
...
--Спроба вставлення запису до `Professors` з існуючим віком менше 20, щоб перевірити CHECK обмеження
INSERT INTO Professors (ProfessorID, FirstName, LastName, DepartmentID, Email) 
...
--Введення даних до `Departments` і `Employees`, потім створіть призначення у `Assignments`
  ...
  VALUES (1, 1, 1);
--Спроба вставлення запису до `Assignments` з неіснуючим `EmployeeID` або `DepartmentID`, щоб перевірити FOREIGN KEY обмеження.
  INSERT INTO Assignments (AssignmentID, EmployeeID, DepartmentID) 
