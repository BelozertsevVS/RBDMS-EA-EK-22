...
Враховуйте тільки замовлення на товари з ідентифікатором товарів (PRODUCT), які 
містять «A» другим символом або «0» в будь-якому місці.
...
які були проведені не в 2008 році. Враховуйте тільки замовлення на товари з ідентифікатором товарів (PRODUCT), 
які містять A другим символом або 0 в будь-якому місці.
- Використовується таблиця [dbo].[ORDERS]
- Задійте предикат LIKE
- Задійте предикат NOT BETWEEN
...
*/
SELECT [MFR], 
       COUNT(DISTINCT [ORDER_NUM]) AS "Кількість унікальних замовлень",
